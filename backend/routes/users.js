...
//        return res.status(409).json({message: "The User with: " + Email+ " is registerd"})
//       }
//      else{
...
//        });
//         newUser.save()
//         .then(() => SendMail(Email, validString, "users"))
...
//     if(err){ res.status(404).json({message: "wrong Email or Password"})}
//     if(user){
//       req.logIn(user, (err) =>{
//         if(err){
...
//   const user = await User.findOne({validString: validstring, "Email" : {
//   $regex : new RegExp(req.body.Email, "i") }})
