...
//     if (isNaN(Phone)){
//       return res.status(400).json({message: "The Phone number should only be numbers"})
//     }

//     if (Name.length < 2){
//       return res.status(401).json({message: "Place Name must be at least 2 charecters"})
//     }
...
//    .then ( place =>{
//       if(place && Name != undefined){
//         return res.status(409).json({message: "The Place with: " + Name + " already exisit in the website"})
//       }
//         else{
//         let holder =  UserHolder(PlaceUser, PlaceOwner, PlaceAdmin);
...
//               result.save()
//              })
//          .then(result => res.status(200).json({message: Name + " Place Added"}))
//         //  .catch(err => res.status(400).json({message: "Error : " +err}))
//         .catch(err => console.log(err))
//         }
//       });
