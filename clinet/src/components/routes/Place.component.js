  ...
  return (
    <div class="card">
      <img
        class=""
        alt="100%x280"
        src={`http://localhost:3000/images/${props}`}
...
export default class Place extends Component {
  ...
  chickowner() {
    axios
      .get("http://localhost:3000/isOwnerregister", { withCredentials: true })
      .then((res) => {
        if (res.data != false) {
          this.setState({
            Owner: true,
  ...
  // async getstars(){
  //   return  await axios.get('http://localhost:3000/places/One/'+this.props.match.params.place)
  //           .then(res => {return
  //             <ReactStars
  //             value={res.data.PlaceInformation.Rating}
  //             count={5}
  //             size={25}
  //             edit={false}
  //             isHalf={false}
  ...
  chickuser() {
    axios
      .get("http://localhost:3000/isUserregister", { withCredentials: true })
      .then((res) => {
        if (res.data != false) {
          this.setState({
            User: true,
  ...
  chickadmin() {
    axios
      .get("http://localhost:3000/isAdminregister", { withCredentials: true })
      .then((res) => {
        if (res.data != false) {
          this.setState({
            Admin: true,
  ...
  async componentDidMount() {
    ...
    await axios
      .get("http://localhost:3000/places/One/" + this.props.match.params.place)
      .then((resp) => {
  ...
  async onSubmit(e) {
    ...
    // formData.append('ReviewPlace', this.state.id);
    ...
    if (!this.state.Owner && !this.state.User && !this.state.Admin) {
      ...
      await axios
        .post("http://localhost:3000/reviews/add", user, {
    ...
    return (
      <div>
        <div
          ...
          style={{ "background-color": "white", marginTop: "2%" }}
        >
            ...
            <div class="container py-3">
              <div class="row">
                <div class="col-md-8 col-sm-6 col-lg-10 row">
                  <img
                    className="placeimg"
                    src={`http://localhost:3000/images/${this.state.Image}`}
                    onClick={() => (window.location = "/")}
                  />
                  <h1 class="display-5 fw-bold place-info">
                    {this.state.Name}
                    <div>{getstars(5)}</div>
                    {/* {this.state.Rating.map(home => <div>{getstars({Rating: home})}</div>)} */}
                  </h1>
                </div>
                <Favorites
          ...
          <div class="row align-items-md-stretch">
            <div class="col-md-6">
              <div class="h-100 p-5 text-white bg-info rounded-3">
                {/* {this.state.Instagram == ''  && <p><FiPhone style={{fontSize: "30px"}}/>{this.state.Phone}</p>} */}
                <p style={{ color: "#fff" }}>
                  <MdDescription style={{ fontSize: "30px" }} />{" "}
                  {this.state.Description}
                </p>
                <p style={{ color: "#fff", letterSpacing: "1px" }}>
                  <RiDoorOpenFill style={{ fontSize: "30px" }} />{" "}
                  {this.state.OpenTime} - {this.state.CloseTime}
                </p>
                <p style={{ color: "#fff", letterSpacing: "3px" }}>
                  {" "}
                  <GiTakeMyMoney style={{ fontSize: "30px" }} />
                  {this.state.PRating}
                </p>
                <p
                  style={{
                    color: "#fff",
                    disply: this.state.Phone < 2 ? "none" : "block",
                  }}
                >
                  {" "}
                ...
                <p
                  style={{
                    color: "#fff",
                    disply: this.state.Instagram == "" ? "none" : "block",
                  }}
                  onClick={() => window.open(this.state.Instagram)}
                >
                  {" "}
                ...
                <p
                  style={{
                    color: "#fff",
                    disply: this.state.Twitter == "" ? "none" : "block",
                  }}
                  onClick={() => window.open(this.state.Twitter)}
                >
                  {" "}
                ...
                <p
                  style={{
                    color: "#fff",
                    disply: this.state.Website == "" ? "none" : "block",
                  }}
                  onClick={() => window.open(this.state.Website)}
                >
                  {" "}
                  <DiChrome style={{ fontSize: "30px" }} /> {this.state.Website}
                </p>
                <p style={{ color: "#fff" }}>
                  {" "}
                  <IoLocationOutline style={{ fontSize: "30px" }} />{" "}
                  {this.state.City} {this.state.Street}, {this.state.ZibCode}
                </p>
              </div>
            </div>
